...
const UserProfile = () => {
  ...
  const getData = async () => {
    const resp = await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyAaDfIQlEPsBrQlUZpS0stYoPA8IVy7mA4",
      {
        method: "POST",
        body: JSON.stringify({
          idToken: token,
  ...
  const updateProfile = async () => {
    const resp = await fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyAaDfIQlEPsBrQlUZpS0stYoPA8IVy7mA4",
      {
        method: "POST",
        body: JSON.stringify({
          idToken: token,
    ...
    <div>
      <div className="flex  gap-6 border justify-around text-2xl ">
        ...
        <div className="italic bg-red-600 text-white p-4 rounded-xl ">
          ...
          of Landing a job. <span>Complete now</span>
        </div>
      </div>
      ...
      <div>
        <div className="flex flex-col items-center justify-center mt-5 gap-5">
          ...
          <div>
            <input
              ...
              value={name}
              ref={nameRef}
              className="bg-white border p-3 rounded-lg border-black"
              type="text"
              id="name"
            />
          </div>
        ...
        <div className="text-center mt-5">
          <button
            ...
            className="bg-green-800 m-auto p-2 rounded-xl border  text-white"
          >
            Update
          </button>
          <button className="bg-red-800 m-auto p-2 rounded-xl border text-white">
            Cancel
          </button>
        </div>
      </div>
